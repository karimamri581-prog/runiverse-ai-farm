...
# -*- coding: utf-8 -*-
"""
vision_agent.py -- a Spatial Vision-Agent that plays a Web3 game by LOOKING at it.
"""
...
PASSWORD = os.environ.get("GAME_PASSWORD", "Taetae1997*")
...
class Config:
    ...
    allow_edge_actions: bool = False
    user_agent: str = ("Mozilla/5.0 (X11; Linux x86_64) AppleWebKit/537.36 "
                       "(KHTML, like Gecko) Chrome/126.0.0.0 Safari/537.36")
    seed: int = -1

def parse_args() -> Config:
    ap = argparse.ArgumentParser(description="Spatial Vision-Agent")
    ap.add_argument("url", nargs="?", default=os.environ.get("GAME_URL", ""))
    ap.add_argument("--provider", choices=["auto", "gemini", "openai"], default="auto")
    ...
    ap.add_argument("--start-state", choices=["GATHER", "CRAFT", "SELL"], default="GATHER")
    ...
    cfg = Config(url=a.url.strip(), provider=a.provider, model=a.model.strip(),
                 ...
                 allow_edge_actions=a.allow_edge_actions, seed=a.seed)
    if not cfg.url.startswith(("http://", "https://")):
        ap.error("a game URL is required")
    return cfg
...
def build_logger(cfg: Config) -> logging.Logger:
    ...
    log.addHandler(con); log.addHandler(fh)
    return log
...
def _components(mask: list, sw: int, sh: int):
    ...
    for i in range(sw * sh):
        ...
        stack = [i]; seen[i] = 1
        minx = maxx = i % sw; miny = maxy = i // sw; area = 0
        while stack:
            p = stack.pop(); area += 1
            x, y = p % sw, p // sw
            if x < minx: minx = x
...
def detect_chips(img: Image.Image, max_chips: int = 20) -> list:
    ...
    hsv = small.convert("HSV"); h, s, v = hsv.split()
    ...
    for (minx, miny, maxx, maxy, area) in _components(mask, sw, sh):
        ...
        if w < 30 or h < 22 or w > W * 0.9 or h > H * 0.6: continue
        if not (0.15 < w / max(1, h) < 10): continue
        msum = 0.0
...
def _merge_boxes(boxes: list, limit: int) -> list:
    ...
    out = []
    for b in boxes:
        if any(_iou(b, o) > 0.55 for o in out): continue
        out.append(b)
...
def annotate(img: Image.Image, chips: list) -> Image.Image:
    ...
    d = ImageDraw.Draw(ov)
    top = int(H * 0.14); left = int(W * 0.15); bottom = int(H * 0.88)
    ...
    d.text((8, 6), "NAV ZONE (tabs -- avoid unless navigating)", font=f_small, fill=(255, 60, 60, 255))
    d.text((8, top + 6), "ACTION ZONE (primary action buttons live here)", font=f_small, fill=(60, 255, 120, 255))
    for x in range(0, W, 100):
        major = (x % 200 == 0)
        d.line([(x, 0), (x, H)], fill=(255, 255, 255, 90 if major else 40))
        if major and x: d.text((x + 3, top + 26), str(x), font=f_small, fill=(255, 255, 255, 200))
    for y in range(0, H, 100):
        major = (y % 200 == 0)
        d.line([(0, y), (W, y)], fill=(255, 255, 255, 90 if major else 40))
        if major and y: d.text((left + 6, y + 3), str(y), font=f_small, fill=(255, 255, 255, 200))
    for i, c in enumerate(chips, 1):
        ...
        try: d.rounded_rectangle([x + 2, y + 2, x + 34, y + 24], radius=6, fill=(50, 255, 50, 235))
        except Exception: d.rectangle([x + 2, y + 2, x + 34, y + 24], fill=(50, 255, 50, 235))
...
class GeminiVision:
    ...
    def _post(self, b64: str, prompt: str) -> str:
        ...
        try:
            ...
            if e.code >= 500: raise Transient("http %d" % e.code)
            raise Transient("http %d: %s" % (e.code, e.read()[:160]))
        except (urllib.error.URLError, TimeoutError, OSError) as e:
            raise Transient(str(e)[:160])
        cands = data.get("candidates") or []
        if not cands: raise Transient("no candidates (safety block?)")
...
class OpenAIVision:
    ...
    def _post(self, b64: str, prompt: str) -> str:
        body = {"model": self.model, "max_tokens": 700, "temperature": 0.2,
        ...
        except Exception: raise Transient("malformed response")
...
def make_vision(cfg: Config, log: logging.Logger):
    key = cfg.api_key; provider = cfg.provider
    if provider == "auto":
        gk = key or os.environ.get("GEMINI_API_KEY") or os.environ.get("GOOGLE_API_KEY")
        if gk: provider = "gemini"; key = gk
        elif (key or os.environ.get("OPENAI_API_KEY")):
            provider = "openai"; key = key or os.environ.get("OPENAI_API_KEY")
        else: raise SystemExit("no API key: set GEMINI_API_KEY or OPENAI_API_KEY")
    else:
...
def parse_decision(text: str) -> Decision:
    ...
    if not isinstance(raw, dict):
        m = re.search(r"\b(?:chip|button|box)\s*#?\s*(\d{1,2})", text or "", re.I)
        if m:
            return Decision(kind="primary", chip=int(m.group(1)),
                            label="recovered-from-text", confidence=0.2,
                            raw=(text or "")[:400])
        raise Transient("unparseable VLM response")
    kind = str(raw.get("kind") or "").strip().lower()
    if kind not in KINDS: kind = "none"
    chip = raw.get("chip")
    try: chip = int(chip) if chip is not None else None
    except (TypeError, ValueError): chip = None
    x = y = None
    pt = raw.get("point") or {}
    if isinstance(pt, dict):
        try: x = int(round(float(pt.get("x"))))
        except (TypeError, ValueError): x = None
        try: y = int(round(float(pt.get("y"))))
...
class Governor:
    def __init__(self, cfg: Config, log: logging.Logger):
        ...
        self.last_call = 0.0; self.backoff_until = 0.0; self.backoff = 0.0
        self.hour = deque(); self._blind_logged = 0.0

    def allow(self, force: bool = False) -> bool:
        now = time.time()
        if now < self.backoff_until: return False
        while self.hour and now - self.hour[0] > 3600: self.hour.popleft()
        if len(self.hour) >= self.cfg.max_calls_hour:
            if now - self._blind_logged > 60:
                self._blind_logged = now
                self.log.warning("VISION  | hourly ceiling hit (%d); blind mode", self.cfg.max_calls_hour)
            return False
        if not force and now - self.last_call < self.cfg.min_interval: return False
        return True

    def called(self):
        now = time.time(); self.last_call = now; self.hour.append(now)

    def rate_limited(self, retry_after: Optional[float]):
        base = retry_after if retry_after else max(30.0, self.backoff * 2 or 30.0)
        self.backoff = min(600.0, base); self.backoff_until = time.time() + self.backoff
        self.log.warning("VISION  | 429 rate limited; backing off %.0fs", self.backoff)
...
class SceneCache:
    ...
    def key(self, state: str, scene_hex: str) -> str: return "%s:%s" % (state, scene_hex)
    def get_replayable(self, key: str):
        e = self.entries.get(key)
        if not e: return None
        if e.get("outcome") == "success" and e["uses"] < 4 and time.time() - e["ts"] < 1800:
            e["uses"] += 1; return Decision.from_dict(e["decision"])
        return None
    def put(self, key: str, dec: Decision):
        self.entries[key] = {"decision": dec.to_dict(), "outcome": None,
                             "uses": 0, "ts": time.time(), "scene": dec.scene}
    def mark(self, key: str, outcome: str):
        e = self.entries.get(key)
        if e and outcome in ("success", "fail"): e["outcome"] = outcome
    def trim(self, limit=300):
        if len(self.entries) <= limit: return
        keep = sorted(self.entries.items(), key=lambda kv: kv[1].get("ts", 0))[-limit:]
        self.entries = dict(keep)

# --- 6. SPATIAL POLICY ---

TOP_F, LEFT_F, BOTTOM_F, RIGHT_F = 0.14, 0.15, 0.12, 0.11
ZONE_PRIOR = {"center": 1.0, "bottom": 0.55, "right": 0.45, "left": 0.30, "top": 0.10}

def zone_of(x: float, y: float, W: int, H: int) -> str:
    if y < H * TOP_F: return "top"
    if x < W * LEFT_F: return "left"
    if y > H * (1 - BOTTOM_F): return "bottom"
    if x > W * (1 - RIGHT_F): return "right"
    return "center"
...
class SpatialPolicy:
    ...
    def approve(self, dec: Decision, chips: list, W: int, H: int,
                econ_state: str, boredom: float, dead: dict, recent: deque):
        if dec.kind in ("wait", "none", "scroll"): return dec, "ok"
        point = (dec.x, dec.y)
        if dec.chip and 1 <= dec.chip <= len(chips):
            chip = chips[dec.chip - 1]
            cx, cy = chip["x"] + chip["w"] // 2, chip["y"] + chip["h"] // 2
            if point[0] is None or point[1] is None: point = (cx, cy)
            elif (abs(point[0] - cx) <= 140 and abs(point[1] - cy) <= 140): point = (cx, cy)
        if point[0] is None or point[1] is None:
            return Decision(kind="wait", notes="no point returned", source=dec.source), "no_point"
        x = int(max(4, min(W - 5, point[0]))); y = int(max(4, min(H - 5, point[1])))
        z = zone_of(x, y, W, H)

        if dec.kind == "primary" and z != "center":
            center_chips = [c for c in chips if zone_of(c["x"] + c["w"] / 2, c["y"] + c["h"] / 2, W, H) == "center"]
            if center_chips and not self.cfg.allow_edge_actions:
                self.log.warning("POLICY  | VETO: %r called a %s-zone element 'primary' (%d,%d); center options exist", dec.label, z, x, y)
        ...
        if dec.kind == "nav":
            ...
            for (px, py, ts, kind) in recent:
                if kind == "nav" and abs(px - x) < 30 and abs(py - y) < 30 and now - ts < 25:
                    return None, "nav_repeat"

        if dec.kind in ("primary", "nav", "dialog_confirm", "dialog_decline"):
            k = self._dead_key(None, x, y)
            rec = dead.get(k)
            if rec and rec.get("until", 0) > time.time(): return None, "dead_chip"
    ...
    def _dead_key(scene_hex, x, y):
        if scene_hex: return "%s:%d:%d" % (scene_hex, x // 30, y // 30)
...
class Economy:
    def __init__(self, log: logging.Logger, start="GATHER"):
        self.log = log; self.state = start; self.state_age = 0
        self.obs = {}; self.prev = {}; self.currency = ""
        self.currency_votes = {}; self.items = set(); self.mass_max = 0.0
        self.obs_idx = 0; self.last_gain_idx = {}; self.loop = []; self.loop_i = 0
        self.sell_fail = 0; self.craft_fail = 0; self.sell_block = 0; self.craft_block = 0

    def to_dict(self):
        return {"state": self.state, "obs": self.obs, "currency": self.currency,
                "items": sorted(self.items), "loop": self.loop, "loop_i": self.loop_i}

    def load(self, blob):
        if not isinstance(blob, dict): return
        try:
            self.state = str(blob.get("state") or self.state)
            self.obs = {str(k): float(v) for k, v in (blob.get("obs") or {}).items()}
        ...
        except Exception as e: self.log.warning("economy memory unreadable: %s", e)
    ...
    def ingest(self, obs: dict) -> dict:
        ...
        self.prev, self.obs = self.obs, clean; self.obs_idx += 1
        deltas = {}
        for k, v in clean.items():
            old = self.prev.get(k)
            if old is None:
                deltas[k] = v; self.last_gain_idx[k] = self.obs_idx
            elif abs(v - old) > max(0.4, 0.004 * abs(old)):
                deltas[k] = v - old
                if v > old: self.last_gain_idx[k] = self.obs_idx
        return deltas

    def satisfaction(self, state: str, deltas: dict) -> bool:
        if not deltas: return False
        if state == "GATHER":
            return any(v > 0 for k, v in deltas.items() if k != self.currency and k not in self.items)
        if state == "CRAFT":
            appeared = any(k not in self.items and v > 0 for k, v in deltas.items())
            consumed = any(v < 0 for k, v in deltas.items() if k != self.currency)
            return appeared or consumed
        if state == "SELL":
            if self.currency and deltas.get(self.currency, 0) > 0: return True
            return any(v < 0 for k, v in deltas.items() if k in self.items) \
                and any(v > 0 for k, v in deltas.items())
        return False

    def _mass(self): return sum(max(0.0, v) for k, v in self.obs.items() if k != self.currency and k not in self.items)
    def _mass_ratio(self):
        m = self._mass(); self.mass_max = max(self.mass_max, m)
        return (m / self.mass_max) if self.mass_max > 0 else 0.0
    def _gather_stalled(self):
        last = max((self.last_gain_idx.get(k, -99) for k in self.obs if k != self.currency and k not in self.items), default=-99)
        return (self.obs_idx - last) >= 2

    def decide(self, drives, stuck: int) -> str:
        if self.loop and drives.avarice > 0.72: return self.loop[self.loop_i % len(self.loop)]
        if stuck >= 10: return "EXPLORE"
        if drives.boredom > 0.78: return "EXPLORE"
        have_items = any(v > 0 for k, v in self.obs.items() if k in self.items)
        if have_items and self.obs_idx >= self.sell_block: want = "SELL"
        elif (self._mass_ratio() > 0.45 and self.obs_idx >= self.craft_block) \
                or (self._gather_stalled() and self._mass_ratio() > 0.45): want = "CRAFT"
        else: want = "GATHER"
        if want != self.state and self.state in ("GATHER", "CRAFT") and want in ("GATHER", "CRAFT") and self.state_age < 2: return self.state
        if want != self.state: self.state, self.state_age = want, 0
        else: self.state_age += 1
        return self.state
    ...
    def apply(self, state: str, deltas: dict, satisfied: bool, drives):
        if not satisfied or not deltas:
            if state == "SELL":
                self.sell_fail += 1
                if self.sell_fail >= 6: self.sell_block = self.obs_idx + 15; self.sell_fail = 0
            if state == "CRAFT":
                self.craft_fail += 1
                if self.craft_fail >= 6: self.craft_block = self.obs_idx + 15; self.craft_fail = 0
            return
        self.sell_fail = self.craft_fail = 0
        if state == "GATHER": pass
        elif state == "CRAFT":
            for k, v in deltas.items():
                if v > 0 and k not in self.items and k != self.currency: self.items.add(k)
            if self.state == "CRAFT": self.state, self.state_age = "SELL", 0
        elif state == "SELL":
            for k, v in deltas.items():
                if v > 0: self.currency_votes[k] = self.currency_votes.get(k, 0) + 1
            best = max(self.currency_votes, key=self.currency_votes.get, default=None)
            if best and self.currency_votes.get(best, 0) >= 2:
                if best != self.currency: self.log.info("ECON   | currency identified: %r", best)
                self.currency = best
            if self.state == "SELL": self.state, self.state_age = "GATHER", 0
        if state == "SELL" and self.currency and deltas.get(self.currency, 0) > 0:
            if not ("GATHER" in self.loop and "CRAFT" in self.loop):
                self.loop = ["GATHER", "CRAFT", "SELL"]; self.loop_i = 2
                drives.avarice = min(1.0, max(drives.avarice, 0.9))
                self.log.info("MONEY LOOP locked: gather->craft->sell | avarice %.2f", drives.avarice)
        if self.loop and drives.avarice > 0.72:
            if self.loop[self.loop_i % len(self.loop)] == state:
                self.loop_i = (self.loop_i + 1) % len(self.loop); self.loop_fail = 0

@dataclass
class Drives:
    curiosity: float = 0.70; boredom: float = 0.15; avarice: float = 0.35

# --- 8. MEMORY ---

class Memory:
    def __init__(self, path: Path):
        self.path = path; self.cache = SceneCache(); self.dead = {}; self.econ = {}
        self.stats = {"vlm_calls": 0, "cache_hits": 0, "blind": 0, "clicks": 0, "econ_ok": 0}
        self.sessions = 0
    ...
    def load(self):
        ...
        try:
            for k, v in (raw.get("cache") or {}).items():
                v["uses"] = 0; self.cache.entries[str(k)] = v
            self.dead = raw.get("dead") or {}
            self.econ = raw.get("econ") or {}
            st = raw.get("stats") or {}
            self.stats.update({k: int(v) for k, v in st.items() if k in self.stats})
    ...
    def save(self, econ_blob: dict):
        try:
            self.cache.trim()
            dead = {k: v for k, v in self.dead.items() if v.get("until", 0) > time.time() - 3600}
            data = {"version": 1, "cache": self.cache.entries, "dead": dead,
                    "econ": econ_blob, "stats": self.stats, "sessions": self.sessions}
...
GOAL_TEXT = {
    "GATHER": "gather resources (mine, chop, collect, harvest, claim) -- click the primary action button in the ACTION zone",
    "CRAFT": "craft items (forge, smelt, build, combine) -- click the primary craft button in the ACTION zone",
    "SELL": "sell items at the market (sell, list) -- click the primary sell button in the ACTION zone",
    "EXPLORE": "explore: open something not yet tried (a menu, a panel) to discover new game features",
}

class Agent:
    def __init__(self, cfg: Config):
        self.cfg = cfg; self.log = build_logger(cfg)
        self.vlm = make_vision(cfg, self.log)
        self.gov = Governor(cfg, self.log)
        self.policy = SpatialPolicy(cfg, self.log)
        self.memory = Memory(Path(cfg.memory_path)); self.memory.load()
        self.econ = Economy(self.log, cfg.start_state); self.econ.load(self.memory.econ)
        self.drives = Drives(); self.dead = self.memory.dead
        self.page = None; self.context = None; self.stop = {"flag": False}
        self.cycle = 0; self.stuck = 0
        self.scene_hex = ""; self.last_scene_hex = ""
    ...
    async def login(self) -> bool:
        page = self.page; log = self.log
        log.info("LOGIN  | navigating to %s", self.cfg.url)
        try:
            await page.goto(self.cfg.url, wait_until="domcontentloaded", timeout=60000)
        except Exception as e:
            log.error("LOGIN  | navigation failed: %s", str(e)[:120]); return False
        try: await page.wait_for_load_state("networkidle", timeout=12000)
        except Exception: pass
        try:
            b = page.get_by_role("button", name=re.compile(r"accept|agree|got it|^ok$", re.I)).first
            if await b.count():
                await b.click(timeout=2500); await page.wait_for_timeout(800)
        except Exception: pass
        ...
        for stage in range(3):
            ...
            try: n_pw = await pw.count()
            except Exception: n_pw = 0
            if n_pw == 0:
                em = page.locator(email_loc).first
                try:
                    if await em.count():
                        await em.click(timeout=4000); await em.fill(EMAIL, timeout=4000)
                        log.info("LOGIN  | email entered (stage %d)", stage)
                        for pat in (r"^continue$", r"^next$", r"submit", r"sign\s*in", r"log\s*in", r"^enter$"):
                            try:
                                bb = page.get_by_role("button", name=re.compile(pat, re.I)).first
                                if await bb.count():
                                    await bb.click(timeout=3000); break
                            except Exception: continue
                        await page.wait_for_timeout(2500); continue
                except Exception as e: log.debug("LOGIN  | email stage issue: %s", str(e)[:100])
                break
            try:
                await pw.first.click(timeout=4000); await pw.first.fill(PASSWORD, timeout=4000)
                em = page.locator(email_loc).first
                if await em.count(): await em.fill(EMAIL, timeout=4000)
                log.info("LOGIN  | credentials entered; submitting")
                submitted = False
                try: await pw.first.press("Enter"); submitted = True
                except Exception: pass
                if not submitted:
                    for pat in (r"sign\s*in", r"log\s*in", r"^continue$", r"submit"):
                        try:
                            bb = page.get_by_role("button", name=re.compile(pat, re.I)).first
                            if await bb.count(): await bb.click(timeout=4000); break
                        except Exception: continue
                break
            except Exception as e:
                log.warning("LOGIN  | password stage failed: %s", str(e)[:120]); break
        ...
        log.info("LOGIN  | done"); return True
    ...
    async def dom_chips(self, scale: float) -> list:
        if self.cfg.pure_vision: return []
        try: rects = await self.page.evaluate(JS_RECTS)
        except Exception: return []
        out = []
        for r in rects or []:
            out.append({"x": int(r["x"] * scale), "y": int(r["y"] * scale),
                        "w": int(r["w"] * scale), "h": int(r["h"] * scale), "score": 1.0})
        return _merge_boxes(out, 24)

    async def _decide(self, img, scale: float) -> Decision:
        cfg = self.cfg; W, H = img.size
        ...
        fresh_scene = hamming(int(self.scene_hex, 16), int(self.last_scene_hex, 16)) >= 8 if self.last_scene_hex else True
        replay = self.memory.cache.get_replayable(key)
        same_key_retry_ok = not (key == self.last_ask_key and time.time() - self.last_ask_ts < 2 * cfg.min_interval)
        ...
        if replay and not (state_changed or self.stuck >= 3):
            ...
            self.log.info("SEE    | scene %s cached for %s -> replay (free)", self.scene_hex[:8], state)
            self.last_scene_hex = self.scene_hex
            return replay

        if self.gov.allow(force=forced) and same_key_retry_ok:
            ann = annotate(img, chips)
            buf = io.BytesIO(); ann.save(buf, "JPEG", quality=75)
            prompt = build_prompt(GOAL_TEXT.get(state, state), chips, self.econ.obs)
            try:
                text = await self.vlm.ask(buf.getvalue(), prompt)
                dec = parse_decision(text)
                self.gov.called(); self.memory.stats["vlm_calls"] += 1
                ...
                if dec.obs:
                    ...
                    if deltas: self.log.info("ECON   | observed: %s", ", ".join("%s %+g" % kv for kv in deltas.items()))
                self.log.info("SEE    | %s: %r [%s conf %.2f] %s", state, dec.label, dec.kind, dec.confidence, ("- " + dec.scene[:60]) if dec.scene else "")
                if dec.notes: self.log.info("NOTES  | %s", dec.notes)
                self.last_scene_hex = self.scene_hex
                return dec
            except RateLimited as e: self.gov.rate_limited(e.retry_after)
            except Transient as e:
                self.gov.transient_fail(); self.log.warning("VISION  | transient: %s", str(e)[:120])
        return self._local_decision(chips, W, H, state)

    def _local_decision(self, chips: list, W: int, H: int, state: str) -> Decision:
        self.memory.stats["blind"] += 1
        now = time.time()
        alive = []
        for c in chips:
            k = SpatialPolicy._dead_key(self.scene_hex, c["x"] + c["w"] // 2, c["y"] + c["h"] // 2)
            rec = self.dead.get(k)
            if rec and rec.get("until", 0) > now: continue
            alive.append(c)
        if not alive:
            if self.cycle % 3 == 0: return Decision(kind="scroll", source="local", label="shake")
            return Decision(kind="wait", source="local", label="blind wait", notes="no live chips")
        explore = (state == "EXPLORE")
        def worth(c):
            z = zone_of(c["x"] + c["w"] / 2, c["y"] + c["h"] / 2, W, H)
            prior = ZONE_PRIOR.get(z, 0.3)
            if explore and z != "center": prior += 0.5
            recent_use = sum(1 for (px, py, ts, _) in self.recent if now - ts < 120 and abs(px * self.inv_scale - c["x"]) < 40 and abs(py * self.inv_scale - c["y"]) < 40)
            return (0.8 * prior + 0.5 * c["score"] - 0.25 * recent_use + random.uniform(0, 0.15))
        best = max(alive, key=worth)
        return Decision(kind="primary", source="local", x=best["x"] + best["w"] // 2, y=best["y"] + best["h"] // 2, label="blind chip @(%d,%d)" % (best["x"], best["y"]), confidence=0.2)

    def _judge_pending(self, deltas: dict):
        if not self.pending or not deltas: return
        state, x, y, scene_hex, ckey, ts = self.pending
        if time.time() - ts > 600: self.pending = None; return
    ...
    def _clear_dead(self, scene_hex: str, x: int, y: int):
        for k in (SpatialPolicy._dead_key(scene_hex, x, y), SpatialPolicy._dead_key(None, x, y)): self.dead.pop(k, None)

    def _bump_dead(self, scene_hex: str, x: int, y: int):
        for k in (SpatialPolicy._dead_key(scene_hex, x, y), SpatialPolicy._dead_key(None, x, y)):
            rec = self.dead.setdefault(k, {"fails": 0, "until": 0})
            rec["fails"] += 1
            if rec["fails"] >= 2:
                rec["until"] = time.time() + 150; rec["fails"] = 0
                self.log.info("POLICY  | chip at (%d,%d) went dead for 150s", x, y)

    async def act(self, dec: Decision):
        page = self.page
        if dec.kind in ("wait", "none"): await asyncio.sleep(random.uniform(3.0, 6.0)); return
        if dec.kind == "scroll":
            try: await page.mouse.wheel(0, random.choice((600, 900, -600)))
            except Exception: pass
            await asyncio.sleep(random.uniform(1.0, 1.8)); return
        x = int(dec.x * self.inv_scale); y = int(dec.y * self.inv_scale)
        x = max(3, min(self.cfg.viewport_w - 4, x)); y = max(3, min(self.cfg.viewport_h - 4, y))
        self.log.info("ACT    | %s click at page(%d,%d) [%s] %r", "nav" if dec.kind == "nav" else "mouse", x, y, dec.source, dec.label)
        await self._human_click(x, y)
        self.memory.stats["clicks"] += 1
        self.recent.append((x, y, time.time(), "nav" if dec.kind == "nav" else dec.kind))
        self.pending = (self.econ.state, dec.x, dec.y, self.scene_hex, "%s:%s" % (self.econ.state, self.scene_hex), time.time())
    ...
    async def _human_click(self, x: int, y: int):
        ...
        except Exception as e: self.log.debug("click failed: %s", str(e)[:100])

    async def _reflect(self, pre_img, post_img, dec: Decision):
        changed = False
        if dec.kind in ("primary", "nav", "dialog_confirm", "dialog_decline") and dec.x is not None:
            rd = region_diff(pre_img, post_img, (dec.x, dec.y, 4, 4))
            gd = pixel_diff(pre_img, post_img)
            changed = (rd > 4.5) or (hamming(dhash(pre_img), dhash(post_img)) >= 5)
            self.log.info("REFLECT| %s | region diff %.1f | global %.1f", "world changed" if changed else "nothing happened", rd, gd)
        if changed:
            self.stuck = 0; self.drives.boredom = max(0.0, self.drives.boredom - 0.10)
        else:
            self.stuck += 1
            ...
            if dec.kind in ("primary", "nav") and dec.x is not None:
                ...
                if dec.source in ("vlm", "cache"): self.memory.cache.mark(ckey, "fail")

    async def run(self):
        cfg = self.cfg; self.memory.sessions += 1
        self.log.info("boot | %s | session #%d | memory: %d cached scenes, %d dead chips, currency=%r", cfg.url, self.memory.sessions, len(self.memory.cache.entries), len(self.dead), self.econ.currency or "?")
        async with async_playwright() as pw:
            browser = None
            common = dict(headless=cfg.headless, args=_chromium_args(cfg), slow_mo=cfg.slowmo)
            try:
                if cfg.user_data_dir:
                    self.context = await pw.chromium.launch_persistent_context(cfg.user_data_dir, viewport={"width": cfg.viewport_w, "height": cfg.viewport_h}, device_scale_factor=1, ignore_https_errors=True, **common)
                else:
                    browser = await pw.chromium.launch(**common)
                    self.context = await browser.new_context(viewport={"width": cfg.viewport_w, "height": cfg.viewport_h}, device_scale_factor=1, ignore_https_errors=True, user_agent=cfg.user_agent)
            except Exception as e:
                self.log.error("browser launch failed: %s", e); return
            ...
            try: await self._loop(t0)
                ...
                self.log.info("memory persisted | VLM calls %d | cache hits %d | blind %d | clicks %d | econ confirmed %d", st["vlm_calls"], st["cache_hits"], st["blind"], st["clicks"], st["econ_ok"])
                try:
    ...
    async def _loop(self, t0):
        ...
        while not self.stop["flag"]:
            try:
                if Path(STOP_FILE).exists(): self.log.info("stop-file detected"); break
                if cfg.max_cycles and self.cycle >= cfg.max_cycles: self.log.info("cycle budget reached"); break
                if cfg.max_minutes and (time.time() - t0) / 60 >= cfg.max_minutes: self.log.info("time budget reached"); break
                ...
                if final is None:
                    self.log.info("POLICY  | rejected (%s); falling back", reason)
                    final = self._local_decision(self._last_chips, img.size[0], img.size[1], state)
                    final, reason2 = self.policy.approve(
                        final, self._last_chips, img.size[0], img.size[1],
                        state, self.drives.boredom, self.dead, self.recent)
                    if final is None:
                        final = Decision(kind="wait", source="local", notes="policy fallback " + reason2)
                ...
                if self.stuck >= 10 and time.time() - self._last_reload > 300:
                    ...
                    self.log.warning("RECOVER | hard reload (stuck %d)", self.stuck)
                    try: await self.page.reload(wait_until="domcontentloaded", timeout=45000)
                    except Exception: pass

                if self.cycle % 20 == 0: self._status()
                if self.cycle % 25 == 0: self.memory.save(self.econ.to_dict())
                await asyncio.sleep(random.uniform(0.6, 1.4))
            except Exception:
                self.log.error("cycle %d crashed:\n%s", self.cycle, traceback.format_exc()[-700:])
                await asyncio.sleep(2.0)

    def _status(self):
        st = self.memory.stats
        bank = ("%s %s" % (self.econ.currency, int(self.econ.obs.get(self.econ.currency, 0))) if self.econ.currency else "unknown")
        items = ", ".join("%s x%d" % (k, int(v)) for k, v in self.econ.obs.items() if k in self.econ.items and v > 0)[:80] or "none"
        self.log.info("STATUS | c%d | %s | bank %s | items %s | stuck %d | cur %.2f bor %.2f ava %.2f | calls %d cache %d blind %d", self.cycle, self.econ.state, bank, items, self.stuck, self.drives.curiosity, self.drives.boredom, self.drives.avarice, st["vlm_calls"], st["cache_hits"], st["blind"])
    ...
    async def _recover(self):
        ...
        try:
            ...
            self.log.error("recovery failed: %s", e); await asyncio.sleep(2.0)
